...
def process_directory(directory):
    ...
    for filename in os.listdir(directory):
        if filename.endswith('.camrange.cub.txt') and re.match(r'M\d{9}(LE|RE)\.camrange\.cub\.txt', filename):
...
for i, inter in enumerate(intersections):
    ...
    print(f"Intersection between {base_name} and the Connecting Ridge: {inter}")
    
    # Output the areas of the individual intersections with the connecting ridge
for idx, area in enumerate(intersection_areas):
    print(f"Area of intersection between {intersection_base_names[idx]} and the landing site: {area}")
...
ax.set_title('Connecting Ridge intersecting the image boxes')
#ax.legend()

# Generate a timestamp for the filename
filename_img1 = f"Connecting_Ridge_intersect_img_boxes_{timestamp}.png"
...
pattern = re.compile(r'M\d{9}(LE|RE)')
    ...
    for j in range(i + 1, len(intersections)):
        ...
        if not inter.is_empty:
            ...
            if (pattern.match(base_name_1) and pattern.match(base_name_2) and 
                base_name_1[:-2] == base_name_2[:-2]):  # Check if they have the same nine digits
...
for idx, inter in enumerate(intersection_of_intersections):
    ...
    print(f"Intersection {idx + 1} of {base_name_1} and {base_name_2} inside the Connecting Ridge cambox: {inter}")
    print(f"Centroid of intersection {idx + 1} of {base_name_1} and {base_name_2} inside the Connecting Ridge cambox: {centroids[idx]}")
    print(f"Area of intersection {idx + 1} of {base_name_1} and {base_name_2} inside the Connecting Ridge cambox: {intersection_of_intersection_areas[idx]}")
...
filename_csv = f'acceptable_img_pairs.csv'
...
# Generate a timestamp for the filename
filename_img2 = f"Img_boxes_intersect_img_boxes_{timestamp}.png"
...
logging.info(f"Graph of the site intersecting the image boxes is {filename_img1}")
logging.info(f"Graph of image boxes intersecting the image boxes inside the site is {filename_img2}")
